...
@pytest.mark.parametrize("moves_to_make", [1, 10])
...
@pytest.mark.parametrize("visit_count", [1, 10])
def test_ship_counts_visits(visit_count):
    sut = seafarers.Ship()
    island = seafarers.Island(0, 0)
    for visit in range(0, visit_count):
        sut.dock(island)
